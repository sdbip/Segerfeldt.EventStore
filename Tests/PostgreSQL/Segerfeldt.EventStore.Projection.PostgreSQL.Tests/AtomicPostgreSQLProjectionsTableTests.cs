...
public sealed class AtomicPostgreSQLProjectionsTableTests
{
    private readonly string? connectionString = Environment.GetEnvironmentVariable("POSTGRES_TEST_CONNECTION_STRING");

    private AtomicPostgreSQLProjectionsTable table = null!;
    ...
    public void SetUp()
    {
        ...
        AtomicPostgreSQLProjectionsTable.AddSchema(connection);
        database = new TargetDatabase(CreateConnection);
        table = new AtomicPostgreSQLProjectionsTable("source", new MockServiceProvider(database));
    }
