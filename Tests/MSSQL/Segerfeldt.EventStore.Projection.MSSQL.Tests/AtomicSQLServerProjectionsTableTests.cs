...
public sealed class AtomicSQLServerProjectionsTableTests
{
    private readonly string? connectionString = Environment.GetEnvironmentVariable("MSSQL_TEST_CONNECTION_STRING");

    private AtomicSQLServerProjectionsTable table = null!;
    ...
    public void SetUp()
    {
        ...
        AtomicSQLServerProjectionsTable.AddSchema(connection);
        database = new TargetDatabase(CreateConnection);
        table = new AtomicSQLServerProjectionsTable("source", new MockServiceProvider(database));
    }
