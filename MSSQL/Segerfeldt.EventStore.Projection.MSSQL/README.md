...
You will need to set up a database connection for each write-model database (a.k.a. `EventSource`) you want to project state from. Call the overloaded extension method `IServiceCollection.AddHostedSQLServerEventSource()` to subscribe to a SQL Server write-model:

```c#
builder.Services.AddSingleton<MyCustomProjectionTracker>();
builder.Services.AddHostedSQLServerEventSource("source", builder.Configuration.GetConnectionString("source_database")!, new EventSourceOptions
{
