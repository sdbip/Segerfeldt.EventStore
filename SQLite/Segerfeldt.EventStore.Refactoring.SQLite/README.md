...
# Segerfeldt.EventStore.Refactoring.SQLite
...
You will need to set up a database connection for each write-model database (a.k.a. `EventSource`) you want to project state from. Call the extension method `IServiceCollection.UseSQLiteRefactoring(string)` to subscribe to a SQLite write-model:

```csharp
builder.Services.UseSQLiteRefactoring(builder.Configuration.GetConnectionString("old-event-model")!)
...
You do not have to have SQLite on both sides of the refactoring. You can for example copy an SQLite event model to your custom database:
...
Or you could copy from your custom database to SQLite:

```csharp
builder.Services.UseRefactoring(p => new MyCustomConnection(builder.Configuration.GetConnectionString("old-event-model")!))
    .UseSQLiteTarget(builder.Configuration.GetConnectionString("new-event-model")!)
    .UseProjectionTracker<MyCustomProjectionTracker>()
    .UseTransformation<MyCustomTransformationStrategy>();
```
...
```csharp
builder.Services.UseSQLiteRefactoring(builder.Configuration.GetConnectionString("new-event-model")!)
    .UseSQLiteTarget(builder.Configuration.GetConnectionString("old-event-model")!)
