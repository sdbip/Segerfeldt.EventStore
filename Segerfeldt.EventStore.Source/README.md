...
# Segerfeldt.EventStore.Source

A NuGet package for implementing the Command/Write-Model side of a CQRS application with your custom database.

Add Segerfeldt.EventStore.Projection to another project to sync the state to a Query/Read-Model database.
...
You will need to set up a database connection so that the commands know where to read/write their data. If you don't want to define your own provider there are three packages too choose from for built-in database support.
...
{
    protected override async Task<CommandResult> Execute(IncrementCounter command)
    {
        ...
        var counter = await EntityStore.ReconstituteAsync<Counter>(id, Counter.EntityType);
