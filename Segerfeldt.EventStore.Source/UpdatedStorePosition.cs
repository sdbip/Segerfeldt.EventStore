...
public sealed class UpdatedStorePosition(Position position, IEnumerable<(EntityId id, Ordinal version)> entityVersions)
{
    /// <summary>the new position after the updaate</summary>
    public Position Position { get; } = position;

    private readonly IReadOnlyDictionary<EntityId, Ordinal> entityVersions = entityVersions.ToImmutableDictionary(x => x.id, x => x.version);
    ...
    /// <returns></returns>
    public Ordinal? GetVersion(EntityId entityId) =>
