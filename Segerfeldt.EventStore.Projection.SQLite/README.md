...
builder.Services.AddHostedSQLiteEventSource(builder.Configuration.GetConnectionString("source_database")!)
...
internal class MyCustomEventSourceProvider(IConfiguration configuration) : IEventSourceProvider
{
    ...
    public void PrepareToReceive(IServiceProvider p)
    {
        ...
        MySchema.CreateIfMissing(new SQLiteConnection(configuration.GetConnectionString("projection_database")!));
    ...
    public DbConnection CreateConnection() => new SQLiteConnection(configuration.GetConnectionString("source_database")!);
}
```
