...
# Segerfeldt.EventStore.Refactoring.MySQL
...
You will need to set up a database connection for each write-model database (a.k.a. `EventSource`) you want to project state from. Call the extension method `IServiceCollection.UseMySQLRefactoring(string)` to subscribe to a MySQL write-model:

```csharp
builder.Services.UseMySQLRefactoring(builder.Configuration.GetConnectionString("old-event-model")!)
    .UseMySQLTarget(builder.Configuration.GetConnectionString("new-event-model")!)
    .UseProjectionTracker<MyCustomProjectionTracker>()
    .UseTransformation<MyCustomTransformationStrategy>();
```

You do not have to have MySQL on both sides of the refactoring. You can for example copy an MySQL event model to your custom database:
...
```csharp
builder.Services.UseMySQLRefactoring(builder.Configuration.GetConnectionString("new-event-model")!)
    .UseMySQLTarget(builder.Configuration.GetConnectionString("old-event-model")!)
