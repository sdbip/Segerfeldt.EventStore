...
# Segerfeldt.EventStore.Projection.MySQL
...
You will need to set up a database connection for each write-model database (a.k.a. `EventSource`) you want to project state from. Call the overloaded extension method `IServiceCollection.AddHostedMySQLEventSource()` to subscribe to a MySQL write-model:

```c#
builder.Services.AddHostedMySQLEventSource("source", builder.Configuration.GetConnectionString("source_database")!, new EventSourceOptions
{
    ...
    .SetMySQLTarget(builder.Configuration.GetConnectionString("target_database")!)
    .AddReceptacles()
    .SetProjectionTracker<MyCustomProjectionTracker>();
```

You can add multiple sources (and they don't all have to be MySQL databases). Just make sure that they are logically separated in the projection database (or use transactions) as they will emit events on independent threads:

```c#
builder.Services.AddHostedMySQLEventSource("source-1", builder.Configuration.GetConnectionString("source1_database")!)
    .SetMySQLTarget(builder.Configuration.GetConnectionString("target_database")!)
    .AddReceptacles()
    .SetProjectionTracker<Source1ProjectionTracker>();

builder.Services.AddHostedPostgreSQLEventSource("source-2", builder.Configuration.GetConnectionString("source2_database")!)
    .SetMySQLTarget(builder.Configuration.GetConnectionString("target_database")!)
    .AddReceptacles()
    .SetProjectionTracker<Source2ProjectionTracker>();

builder.Services.AddHostedSQLServerEventSource("source-3", builder.Configuration.GetConnectionString("source3_database")!)
    .SetMySQLTarget(builder.Configuration.GetConnectionString("target_database")!)
    .AddReceptacles()
    .SetProjectionTracker<Source3ProjectionTracker>();

builder.Services.AddHostedSQLiteEventSource("source-4", builder.Configuration.GetConnectionString("source4_database")!)
    .SetMySQLTarget(builder.Configuration.GetConnectionString("target_database")!)
    .AddReceptacles()
    .SetProjectionTracker<Source4ProjectionTracker>();
```
...
```c#
using Segerfeldt.EventStore.Projection.MySQL;
